...
function App() {
  ...
  useEffect(() => {
    if (commandes.length > 0) {
      setByDateProduct(getByRangeDate(commandes, "dateCom", start, end));
    }
  }, [commandes]);
  useEffect(() => {
    setResult(flatify(byDateProduct, "contenu"));
  }, [byDateProduct]);*/
  }
  {
    /**useEffect(()=>{
  ...
  useEffect(() => {
    if (!meta.isFetching) {
      const val = getByRangeDate(commandes, "dateCom", start, end);
      setByDateProduct(val);
    }
  }, [meta]);
  const getDataa = () => {
                ...
                {/** {resultat.map((b) => (
                  <>
                    <tr>
                      ...
                      {rangeDate.map((r) => (
                        <>
                          <td className="text-center" title={b.name}>
                            {/*{getByRangeDateAndProduct(
                              resultat,
                              "dateCom",
                              r,
                              b.id
