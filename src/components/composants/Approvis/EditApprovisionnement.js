...
const EditApprovisionnement = ({ setRegenerate, approv }) => {
  ...
  const [copyProduct, setCopyProduct] = useState(approv?.contenu);
  ...
  useEffect(() => {
    copyProduct?.map((p) => addToApprov(p));
  }, []);
...
<Card>
      <Card.Header className="d-flex justify-content-between align-items-center bg-dark text-white">
        <div>
          Approvisionenement (
          {` ${approvisionnements?.contenu?.length} ${
            approvisionnements?.contenu?.length > 1 ? "articles" : "article"
          }`}
          )
        </div>
      </Card.Header>
      <Card.Body>
        ...
        {approv?.contenu?.length <= 0 && (
          <div class="alert alert-success ">Aucune produit !!!</div>
        )}

        {approv?.contenu?.map((product, i) => (
          <ApprovisionnementItem
            key={`${product?.id}_${i}`}
            product={product}
            approvisionnement={approvisionnements?.contenu}
            dispatch={dispatch}
          />
        ))}
        {[...approvisionnements]?.contenu?.length > 0 && (
          <>
                ...
                <strong>
                  <NumberFormat
                    value={calculateTotal(
                      approvisionnements?.contenu?.map(
                        (product) =>
            ...
            <div className="d-flex justify-content-end">
              <button
                ...
                disabled={approvisionnements?.contenu?.length === 0}
                onClick={onCheckOut}
                type="button"
              >
                Valider l'operation
